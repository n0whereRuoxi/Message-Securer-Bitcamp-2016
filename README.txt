...
- At recommended JUMPER setting, the encoding rate for mp3 and aac files is about 0.1%, which means for each 10MB the program is able to encode roughly 1250 characters (1250 Bytes) into the file. The encoding rate for wav file is about 1%, which means for each 10MB the program is able to encode roughly 10,000 characters into the file.
